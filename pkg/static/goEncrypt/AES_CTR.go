...
/*
	AES的CTR模式加解密
*/
func AesCTR_Encrypt(plainText, key []byte) ([]byte, error) {
	// 判断用户传过来的key是否符合16字节，如果不符合16字节加以处理
	keylen := len(key)
	if keylen == 0 { // 如果用户传入的密钥为空那么就用默认密钥
		key = []byte("terrysgygoaescbc") // 默认密钥
...
func AesCTR_Decrypt(cipherText, key []byte) ([]byte, error) {
	// 判断用户传过来的key是否符合16字节，如果不符合16字节加以处理
	keylen := len(key)
	if keylen == 0 { // 如果用户传入的密钥为空那么就用默认密钥
		key = []byte("terrysgygoaescbc") // 默认密钥
