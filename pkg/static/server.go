...
// Register publishes in the wuhan the set of methods of the
...
func prepareMethod(method reflect.Method) *methodType {
	...
	default:
		xlog.Logger().Debug("method", mname, "has wrong number of ins:", mtype.NumIn())
		return nil
	}

	// First arg need not be a pointer.
	if !isExportedOrBuiltinType(argType) {
		xlog.Logger().Errorln("", mname, "argument type not exported:", argType)
		return nil
	}
	...
	if replyType.Kind() == reflect.Func {
		...
		if replyType.NumIn() != 1 {
			xlog.Logger().Errorln("method", mname, "sendReply has wrong number of ins:", replyType.NumIn())
			return nil
		}
		if replyType.In(0).Kind() != reflect.Interface {
			xlog.Logger().Errorln("method", mname, "sendReply parameter type not an interface:", replyType.In(0))
			return nil
		}
		if replyType.NumOut() != 1 {
			xlog.Logger().Errorln("method", mname, "sendReply has wrong number of outs:", replyType.NumOut())
			return nil
		}
		if returnType := replyType.Out(0); returnType != typeOfError {
			xlog.Logger().Errorln("method", mname, "sendReply returns", returnType.String(), "not error")
			return nil
		}

	} else if replyType.Kind() != reflect.Ptr {
		xlog.Logger().Errorln("method", mname, "reply type not a pointer:", replyType)
		return nil
	}

	// Reply type must be exported.
	if !isExportedOrBuiltinType(replyType) {
		xlog.Logger().Errorln("method", mname, "reply type not exported:", replyType)
		return nil
	}
	// Method needs one out.
	if mtype.NumOut() != 1 {
		xlog.Logger().Errorln("method", mname, "has wrong number of outs:", mtype.NumOut())
		return nil
	}
	// The return type of the method must be error.
	if returnType := mtype.Out(0); returnType != typeOfError {
		xlog.Logger().Errorln("method", mname, "returns", returnType.String(), "not error")
		return nil
	}
...
func (server *Server) register(rcvr interface{}, name string, useName bool) error {
	...
	if sname == "" {
		xlog.Logger().Errorln("rpc: no service name for type", s.typ.String())
	}
	if !isExported(sname) && !useName {
		s := "rpc Register: type " + sname + " is not exported"
		xlog.Logger().Errorln(s)
		return errors.New(s)
	}
...
// A value sent as a placeholder for the wuhan's response value when the wuhan
...
func (server *Server) sendResponse(sending *sync.Mutex, req *Request, reply interface{}, codec ServerCodec, errmsg string, last bool) (err error) {
	...
	if err != nil {
		xlog.Logger().Errorln("rpc: writing response:", err)
	}
	sending.Unlock()
...
func (s *service) call(server *Server, sending *sync.Mutex, mtype *methodType, req *Request, argv, replyv reflect.Value, codec ServerCodec, context interface{}) {
	...
	if !mtype.stream {
		...
		func() {
			defer func() {
				if r := recover(); r != nil {
					err := fmt.Sprintf("rpcplus call error :%s", r)
					returnValues = append(returnValues, reflect.ValueOf(errors.New(err)))
					xlog.Logger().Errorf("rpcplus call error: %v", r)
					sysdebug.PrintStack()
				}
			}()
	...
	sendReply := func(oneReply interface{}) error {
		...
		if firstType == nil {
			...
			if firstType != typ {
				xlog.Logger().Errorln("passing wrong type to sendReply",
					firstType, "!=", typ)
...
// ServeConn runs the wuhan on a single connection.
...
func (server *Server) ServeCodecWithContext(codec ServerCodec, context interface{}) {
	...
	for {
		...
		if err != nil {
			if err != io.EOF {
				xlog.Logger().Errorln("rpcplus:", err)
			}
			if !keepReading {
				break
			}
...
func (server *Server) readRequestHeader(codec ServerCodec) (service *service, mtype *methodType, req *Request, keepReading bool, err error) {
	...
	if err != nil {
		...
		err = errors.New("rpcplus: wuhan cannot decode request: " + err.Error())
		return
	}
	...
	if len(serviceMethod) != 2 {
		err = errors.New("rpcplus: service/method request ill-formed: " + req.ServiceMethod)
		return
	}
	// Look up the request.
	server.mu.Lock()
	...
	server.mu.Unlock()
	if service == nil {
		err = errors.New("rpcplus: can't find service " + req.ServiceMethod)
		return
	}
	mtype = service.method[serviceMethod[1]]
	if mtype == nil {
		err = errors.New("rpcplus: can't find method " + req.ServiceMethod)
	}
	return
}
...
func (server *Server) Accept(lis net.Listener) {
	for {
		conn, err := lis.Accept()
		if err != nil {
			xlog.Logger().Errorln("rpc.Serve: accept:", err.Error()) // TODO(r): exit?
		}
...
// RPC responses for the wuhan side of an RPC session.
// The wuhan calls ReadRequestHeader and ReadRequestBody in pairs
// to read requests from the connection, and it calls WriteResponse to
// write a response back.  The wuhan calls Close when finished with the
...
func (server *Server) ServeHTTP(w http.ResponseWriter, req *http.Request) {
	...
	if err != nil {
		xlog.Logger().Errorln("rpc hijacking ", req.RemoteAddr, ": ", err.Error())
		return
	}
