...
func HouseReset(header string, data interface{}) (interface{}, *xerrors.XError) {
	// 通知客户端
	value, err := server2.GetServer().CallHall("NewServerMsg", header, data)
	if err != nil {
		xlog.Logger().Errorln("house reset failed:", err.Error())
...
func ResetUserHmRight(header string, data interface{}) (interface{}, *xerrors.XError) {
	// 通知客户端
	value, err := server2.GetServer().CallHall("NewServerMsg", header, data)
	if err != nil {
		xlog.Logger().Errorln("UserHmRight reset failed:", err.Error())
		return nil, xerrors.NewXError(err.Error())
	}
	strVal := fmt.Sprintf("%s", value)
	if strVal != "true" && strVal != "SUC" {
