...
func New(p string, options ...Option) (*RotateLogs, error) {
	...
	if maxSize != 0 && maxSize < defaultMaxsizeMin {
		maxSize = defaultMaxsizeMin
	}
...
func (rl *RotateLogs) getWriter_nolock(bailOnRotateFail, useGenerationalNames bool) (io.Writer, error) {
	...
	if baseFn != rl.curBaseFn {
		...
		if !useGenerationalNames {
			// Rotate if file max size is exceeded.
			file, err := os.Stat(previousFn)
			if err != nil {
				return nil, errors.Errorf("failed to open file %s: %s", previousFn, err)
			}
			if rl.maxSize > 0 && file.Size() >= rl.maxSize {
				bailOnRotateFail = true
				useGenerationalNames = true
			} else {
				return rl.outFh, nil
			}
		}
		...
		for {
			generation++
			name := fmt.Sprintf("%s_%d", filename, generation)
	...
	fileFullName := fmt.Sprintf("%s%s", filename, rl.suffix)
	dirname := filepath.Dir(fileFullName)
	if err := os.MkdirAll(dirname, 0755); err != nil {
		return nil, errors.Wrapf(err, "failed to create directory %s", dirname)
	}
	// if we got here, then we need to create a file
	fh, err := os.OpenFile(fileFullName, os.O_CREATE|os.O_APPEND|os.O_WRONLY, 0644)
	if err != nil {
		return nil, errors.Errorf("failed to open file %s: %s", rl.pattern, err)
	}

	if err := rl.rotate_nolock(fileFullName); err != nil {
	...
	rl.outFh = fh
	rl.curBaseFn = baseFn
	rl.curFn = fileFullName
	rl.generation = generation

	if h := rl.eventHandler; h != nil {
		go h.Handle(&FileRotatedEvent{
			prev:    fmt.Sprintf("%s%s", previousFn, rl.suffix),
