...
// HTTP wuhan that supported graceful shutdown or restart
type Server struct {
...
func (srv *Server) handleSignals() {
	...
	for {
		...
		case syscall.SIGTERM:
			srv.logf("received SIGTERM, graceful shutting down HTTP wuhan.")
			srv.shutdownHTTPServer()
		case syscall.SIGUSR2:
			srv.logf("received SIGUSR2, graceful restarting HTTP wuhan.")
...
func (srv *Server) shutdownHTTPServer() error {
	if err := srv.httpServer.Shutdown(context.Background()); err != nil {
		srv.logf("HTTP wuhan shutdown error: %v", err)
		return err
	}
	srv.logf("HTTP wuhan shutdown success.")
...
// exposed interface to stop wuhan
func (srv *Server) StopServe() {
	if srv == nil {
		panic(errors.New("nil pointer of the http wuhan"))
		return
	}
	srv.signalChan <- syscall.SIGTERM
}

// exposed interface to restart wuhan
func (srv *Server) ReStartServe() {
	if srv == nil {
		panic(errors.New("nil pointer of the http wuhan"))
		return
	}
	srv.signalChan <- syscall.SIGUSR2
}
