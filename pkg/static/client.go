...
func (client *Client) input() {
	...
	for err == nil {
		...
		case call == nil:
			...
			// removed; response is a wuhan telling us about an
	...
	if err != io.EOF && !closing {
		xlog.Logger().Errorln("rpc: client protocol error:", err)
	}
...
func (call *Call) done() {
	...
	default:
		...
		xlog.Logger().Infoln("rpc: discarding Call reply due to insufficient Done chan capacity")
	}
}
...
// DialHTTP connects to an HTTP RPC wuhan at the specified network address
// listening on the default HTTP RPC path.
func DialHTTP(network, address string) (*Client, error) {
	return DialHTTPPath(network, address, DefaultRPCPath)
}

// DialHTTPPath connects to an HTTP RPC wuhan
...
// Dial connects to an RPC wuhan at the specified network address.
...
func (client *Client) Go(serviceMethod string, args interface{}, reply interface{}, done chan *Call) *Call {
	...
	if done == nil {
		...
		if cap(done) == 0 {
			xlog.Logger().Errorln("rpc: done channel is unbuffered")
		}
	}
	call.Done = done
	client.send(call)
...
func (client *Client) StreamGo(serviceMethod string, args interface{}, replyStream interface{}) *Call {
	...
	if typ.Kind() != reflect.Chan || typ.Elem().Kind() != reflect.Ptr {
		xlog.Logger().Errorln("rpc: replyStream is not a channel of pointers")
		return nil
	}
